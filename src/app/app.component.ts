...
@Component({
  ...
  template: `
    ...
    @if (errorMsg()) {
      <div class="toast-error-banner" (click)="clearError()">
        <div class="toast-icon"><app-icon name="warning" [size]="24" /></div>
        <div class="toast-content">
          <div class="toast-title">Lỗi hệ thống</div>
          <div class="toast-desc">{{ errorMsg() }}</div>
        </div>
        <button class="toast-close" type="button"><app-icon name="close" [size]="20" /></button>
      </div>
    }
    @if (successMsg()) {
      <div class="toast-success-banner" (click)="clearSuccess()">
        <div class="toast-icon"><app-icon name="check_circle" [size]="24" /></div>
        <div class="toast-content">
  ...
  styles: [`
    ...
    .toast-title { font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
    .toast-desc { font-size: 12.5px; opacity: 0.95; line-height: 1.4; }
    .toast-close {
      background: none; border: none; color: white; cursor: pointer; opacity: 0.8;
